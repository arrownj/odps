...
-   功能说明
    ...
    -   不指定`<KEY>=<VALUE>`，显示当前项目空间的属性配置。
        ...
        |属性名称|角色|描述|取值范围|
        |:---|:-|:-|:---|
        |odps.sql.allow.fullscan|Project Owner|设置项目空间是否允许全表扫描。|        -   True：允许
        -   False：禁止 |
        |odps.table.drop.ignorenonexistent|所有用户|当删除不存在的表时，是否报错。|        -   True：不报错
        -   False：报错 |
        |odps.table.lifecycle|Project Owner|        -   Optional：创建表时，Lifecycle子句为可选设置，如果不设置生命周期，则该表永久有效。
...
|        -   Optional
        ...
        |READ\_TABLE\_MAX\_ROW|Project Owner|设置SELECT语句返回的数据条数。|1~10000|
